...
def mock_outgoing_web_request(request: Request):
    ...
    with tracer:
        ...
        try:
            ...
            response2 = requests.post('http://localhost:8001/appMiddlewareService', params={'strtag': tag})
            if response2.status_code == 200:
                print("Successfully called appMiddlewareService")
            else:
                print(f"Failed to call appMiddlewareService: {response2.status_code}")
